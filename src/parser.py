...
class Nodo:
    ...
    @staticmethod
    def hoja(lexema): return Nodo(tipo="TERMINAL",valor=lexema)

class ErrorSintactico(Exception):
    def __init__(self,esperado,token):
        self.esperado=esperado; self.token=token
        self.linea=token.linea; self.columna=token.columna
        super().__init__(str(self))
    def __str__(self):
        return f"Error sintáctico en L{self.linea}:C{self.columna} — se esperaba {self.esperado}, se encontró '{self.token.lexema}'"
    def to_dict(self):
        return {"mensaje":str(self),"esperado":self.esperado,"encontrado":self.token.lexema,"tipo_encontrado":self.token.tipo,"linea":self.linea,"columna":self.columna}

_INICIO_SENT = {TipoToken.PACIENTE.value,TipoToken.RUTINA.value,TipoToken.DIETA.value,
                TipoToken.TK_SI.value,TipoToken.TK_IMPRIMIR.value,TipoToken.EDAD.value,
                TipoToken.PESO.value,TipoToken.OBJETIVO.value,TipoToken.RESTRICCION.value,TipoToken.ACCION.value}

class Parser:
    def __init__(self,tokens):
        self._tokens=[t for t in tokens if t.tipo!="TK_EOF"]
        self._eof=next((t for t in tokens if t.tipo=="TK_EOF"),Token("TK_EOF","EOF",0,-1,"otro"))
        self._pos=0
    def _actual(self):
    ...
    def _avanzar(self):
        ...
        return t
    def _verificar(self,*tipos): return self._actual().tipo in tipos
    def _consumir(self,tipo,desc=""):
        t=self._actual()
        if t.tipo==tipo: return self._avanzar()
        raise ErrorSintactico(desc or tipo,t)
    def _hoja(self,t): return Nodo(tipo="TERMINAL",valor=t.lexema)
    def parsear(self):
        try: return self._programa(),None
        except ErrorSintactico as e: return None,e
    def _programa(self):
    ...
    def _bloque_paciente(self):
        ...
        _datos={TipoToken.EDAD.value,TipoToken.PESO.value,TipoToken.OBJETIVO.value,TipoToken.RESTRICCION.value}
    ...
    def _instruccion(self):
        ...
        if t==TipoToken.EDAD.value: return self._instruccion_simple(TipoToken.EDAD.value,TipoToken.TK_ENTERO.value,"número entero")
        if t==TipoToken.OBJETIVO.value: return self._instruccion_simple(TipoToken.OBJETIVO.value,TipoToken.TK_ID.value,"identificador")
        if t==TipoToken.PESO.value: return self._instruccion_numero(TipoToken.PESO.value)
        raise ErrorSintactico("EDAD, PESO, OBJETIVO o RESTRICCION",self._actual())
    def _instruccion_simple(self,kw,tipo_val,desc):
        ...
        nodo.agregar(self._hoja(self._consumir(TipoToken.TK_FIN_INSTRUC.value,"';'")))
        return nodo
    def _instruccion_numero(self,kw):
        nodo=Nodo(tipo="INSTRUCCION")
        nodo.agregar(self._hoja(self._consumir(kw)))
        nodo.agregar(self._hoja(self._consumir(TipoToken.TK_ASIGNACION.value,"':'")))
        t=self._actual()
        ...
        else: raise ErrorSintactico("número",t)
        nodo.agregar(self._hoja(self._consumir(TipoToken.TK_FIN_INSTRUC.value,"';'")))
        return nodo
    ...
    def _accion(self):
        ...
        nodo.agregar(self._hoja(self._consumir(TipoToken.TK_ASIGNACION.value,"':'")))
        nodo.agregar(self._hoja(self._consumir(TipoToken.TK_ID.value,"ejercicio/alimento")))
        if self._verificar(TipoToken.TK_MULT.value):
            nodo.agregar(self._hoja(self._avanzar()))
            nodo.agregar(self._hoja(self._consumir(TipoToken.TK_ENTERO.value,"repeticiones")))
    ...
    def _condicion(self):
        ...
        if t.tipo in (TipoToken.PESO.value,TipoToken.EDAD.value,TipoToken.TK_ID.value):
            nodo.agregar(self._hoja(self._avanzar()))
        else: raise ErrorSintactico("PESO, EDAD o identificador",t)
        t=self._actual()
        if t.tipo in (TipoToken.TK_IGUAL.value,TipoToken.TK_MAYOR.value,TipoToken.TK_MENOR.value,
                      "TK_MAYOR_IGUAL","TK_MENOR_IGUAL",TipoToken.TK_GT.value,TipoToken.TK_LT.value):
            nodo.agregar(self._hoja(self._avanzar()))
        else: raise ErrorSintactico("operador de comparación",t)
        t=self._actual()
        if t.tipo in (TipoToken.TK_ID.value,TipoToken.TK_ENTERO.value,TipoToken.TK_DECIMAL.value):
            nodo.agregar(self._hoja(self._avanzar()))
        else: raise ErrorSintactico("valor",t)
        return nodo
    def _sentencia_imprimir(self):
        ...
        nodo.agregar(self._hoja(self._consumir(TipoToken.TK_ID.value,"nombre bloque")))
        nodo.agregar(self._hoja(self._consumir(TipoToken.TK_FIN_INSTRUC.value,"';'")))
        return nodo
...
def arbol_a_texto(nodo,prefijo="",es_ultimo=True):
    ...
    for i,hijo in enumerate(nodo.hijos):
        linea+=arbol_a_texto(hijo,prefijo+ext,i==len(nodo.hijos)-1)
    return linea

def arbol_a_dict(nodo):
    if nodo.es_hoja(): return {"nombre":nodo.valor,"hijos":[],"es_hoja":True}
